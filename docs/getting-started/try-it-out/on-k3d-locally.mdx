...
| `observer-oauth-client-secret` | `openchoreo-observer-resource-reader-client-secret` | Observer OAuth with ThunderID  |
...
- **ClusterWorkflows**: dockerfile-builder, paketo-buildpacks-builder, gcp-buildpacks-builder, ballerina-buildpack-builder
- **ClusterTraits**: health-probes, horizontal-pod-autoscaler, observability-alert-rule, persistent-volume-claim, vertical-pod-autoscaler
...
Build pipelines are defined as ClusterWorkflowTemplates. Each build workflow (`dockerfile-builder`, `paketo-buildpacks-builder`, etc.) is composed from smaller shared templates: a **checkout step** (controls how source code is cloned), **build coordinator templates** (`containerfile-build`, `paketo-buildpacks-build`, `gcp-buildpacks-build`, `ballerina-buildpack-build`), and a **publish step** (controls where built images get pushed). For k3d, the publish step targets the local registry at `host.k3d.internal:10082`. In a real environment you would point it at ECR, GAR, GHCR, or whatever registry you use.
...
```bash
kubectl wait --for=create --for=condition=available deployment \
